    ...
    return {
      ...
      rules: {
        username: [
          ...
          { min: 2, max: 20, message: '用户名长度应为2到20位', trigger: 'blur' },
          { pattern: /^[A-Za-z0-9]+$/, message: '用户名只能包含数字或字母', trigger: 'blur' }
        ],
        name: [
          { required: true, message: '请输入员工姓名', trigger: 'blur' },
          { min: 2, max: 10, message: '员工姓名长度应为2到10位', trigger: 'blur' }
          // 可以加个正则校验中文，但通常前端校验允许的话用后端最终校验
        ],
        gender: [
          { required: true, message: '请选择性别', trigger: 'change' }
        ],
        job: [
          { required: false } // 职位选填，不强制
        ],
        entrydate: [
          { required: false } // 入职日期选填，不强制
        ],
        deptId: [
          { required: false } // 部门选填，不强制
        ]
      },
      deptList: [],
